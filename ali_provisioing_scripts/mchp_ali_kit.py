...
class MchpAliKitDevice():
    def __init__(self, device):
        super(MchpAliKitDevice, self).__init__()
        self.device = device
        self.report_size = 64
    ...
    def open(self, vendor_id=DEVICE_HID_VID, product_id=DEVICE_HID_PID):
        """Opens HID device for the Ali Kit. Adjusts default VID/PID for the kit."""
        self.app_responses = {}
    ...
    def kit_read_app_no_error(self, id):
        ...
        if resp['error'] is not None:
            raise MchpAliKitError(resp['error'])
        return resp
    ...
    def set_ali_credentials(self, productkey, devicename, devicesecret):
        """Save the Ali credentials to the device."""
        id = self.kit_write_app('setAliCredentials', {'productkey':productkey, 'devicename':devicename, 'devicesecret':devicesecret})
        id = self.kit_read_app_no_error(id)
...
class MchpAliKitError(Exception):
    def __init__(self, error_info):
        self.error_code = error_info['error_code']
        self.error_msg  = error_info['error_msg']
        super(MchpAliKitError, self).__init__('Kit error %d: %s' % (self.error_code, self.error_msg))
